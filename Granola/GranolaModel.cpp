...
void Granola::operator()(tick t)
{
  ...
  float density = inputs.density * (1 + std::normal_distribution<float>(0., inputs.dens_j_r / 4)(rd) * inputs.dens_j) ;
  ...
  for (int k = 0; k < t.frames; k++)
  {
      ...
      if (trigger) {
        trigger_counter = 0;
        density = inputs.density * (1 + std::normal_distribution<float>(0., inputs.dens_j_r / 4)(rd) * inputs.dens_j);
